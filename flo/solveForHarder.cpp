...
long long main() {
    ...
    while (t--) {
        ...
        for (long long i = 0; i < n; i++) {
            ...
            // Try if extendable with the next element is just 1 step beside the
            // upper one
            if (i + 1 < n && dat[i + 1].first - dat[i].first == 1) {
                ...
                // petals to buy further
                coins = m - pred;
                // Check if coins still valid >= dat[i+1].first
                if (coins >= dat[i + 1].first) {
                    // Valid, then now buy the i+1 petals with maximum available
                    // between valid coins and valid amount
                    k2 = min(coins / (dat[i + 1].first), c_x1);
                    // Update the current max value of petals we bought
                    pred += k2 * (dat[i + 1].first);
                    // Update the remaining amount of i+1 petals after being
                    // bought
                    c_x1 -= k2;
